...
const API_BASE_URL = "https://gae-stock-api.onrender.com";
...
function ThemePeerScores({
  ...
  const hasRealPeerScores = peerScores.length > 0;
  const data = hasRealPeerScores
    ? {
        top: [...peerScores].sort((a, b) => b.score - a.score).slice(0, 5),
        bottom: [...peerScores].sort((a, b) => a.score - b.score).slice(0, 5),
        marketCapTop: [...peerScores]
    ...
    : {
        ...fallbackData,
        marketCapTop: fallbackData.top,
      };
  const activeGroup = themeGroups[activeThemeIndex] || null;
      ...
      <div className="mb-3 flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs font-bold text-slate-500">
            {displayTheme} · {stockName} 기준 · {hasRealPeerScores ? "테마 API 점수" : peerLoading ? "불러오는 중" : "예시 데이터"}
          </p>
          {themeGroups.length > 1 && (
            <div className="mt-3 flex flex-wrap gap-2">
              {themeGroups.slice(0, 2).map((group, idx) => (
                <button
                  key={group.theme}
                  type="button"
                  onClick={() => onSelectThemeIndex(idx)}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    async function loadDailyData() {
      ...
      setDataError("");
      try {
        const response = await fetch(`${API_BASE_URL}/api/daily/${stock.code}`);
  ...
  const chartPeriodLabel = chartMonths === 0.25 ? "1주" : `${chartMonths}개월`;
  ...
  useEffect(() => {
    ...
    async function loadPeerScores() {
      ...
      try {
        setPeerLoading(true);
        const themeResponse = await fetch(`${API_BASE_URL}/api/theme/${stock.code}`);
        ...
        const rows = peers.map((peer) => {
          ...
          const marketCapScore = marketCap > 0 ? Math.min(25, Math.log10(marketCap) * 2.2) : 0;
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/search?q=${encodeURIComponent(key)}`);
      const json = await response.json();
    ...
    <main className="min-h-screen bg-slate-50 p-6 text-slate-950 md:p-10">
      <div className="mx-auto max-w-[1480px] space-y-6">
            ...
            <Card>
              ...
              <div className="mt-4 rounded-2xl border border-slate-200 bg-white p-4">
                <div className="mb-3 flex items-center justify-between gap-3">
                  ...
                  <span className="rounded-full bg-slate-100 px-2 py-1 text-[10px] font-bold text-slate-500">{avgCostPeriod} 추정</span>
                </div>
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between rounded-xl bg-slate-50 p-2"><span>개인</span><b>{stock.avgCost[avgCostPeriod].retail}</b></div>
                  <div className="flex justify-between rounded-xl bg-blue-50 p-2"><span>외국인</span><b>{stock.avgCost[avgCostPeriod].foreign}</b></div>
                  <div className="flex justify-between rounded-xl bg-violet-50 p-2"><span>기관</span><b>{stock.avgCost[avgCostPeriod].institution}</b></div>
                </div>
              </div>
            </Card>
        ...
        <section className="grid gap-4 md:grid-cols-5">
          ...
          <BottomTile icon={Activity} label="변동성" value="보통" sub="ATR 기준 예정" />
          <BottomTile icon={Users} label="기관 순매수" value="준비중" sub="수급 API 연결 필요" />
          <BottomTile icon={Globe2} label="외국인 순매수" value="준비중" sub="수급 API 연결 필요" />
          <BottomTile icon={RadioTower} label="프로그램 매매" value="준비중" sub="추후 연결" tone="pink" />
        </section>
